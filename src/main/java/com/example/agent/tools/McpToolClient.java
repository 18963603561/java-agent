...
public class McpToolClient {
    ...
    public McpToolListResponse listTools(McpToolListRequest request, TenantContext tenantContext) {
        ...
        log.info("MCP tools/list end, tenantId={}, serverId={}, count={}",
                tenantContext.getTenantId(), serverId, page.size());
    ...
    public McpToolCallResponse callTool(McpToolCallRequest request, TenantContext tenantContext) {
        ...
        try {
            Map<String, Object> result = toolRegistry.execute(toolName, request.getArguments());
            McpToolCallResponse response = new McpToolCallResponse(
                    request.getCallId(), "SUCCESS", result, null);
            circuitBreakerManager.recordSuccess(rateKey);
            log.info("MCP tools/call success, tenantId={}, serverId={}, toolName={}",
                    tenantContext.getTenantId(), serverId, toolName);
            return response;
